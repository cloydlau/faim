        ...
        <div @click="() => { $emit('click', { index: i, src }) }">
          ...
          >
            <img
              ...
              :height="height"
              :style="{
                cursor: Viewerjs ? 'pointer' : undefined,
              }"
            >
          </slot>
        </div>
...
import { isBase64WithScheme, tryParsingJSONArray } from './utils'
  ...
  computed: {
    ...
    Pattern() {
      return conclude([
        this.pattern,
        globalProps.pattern,
        ['TABLE', 'TR', 'TH', 'TD'].includes(this.$parent?.$el?.tagName)
          ? 'table-cell'
          : undefined,
      ], {
        type: String,
      ...
      if (newFiles.length) {
        this.$nextTick(() => {
          if (this.Pattern === 'swiper' && !this.swiper) {
            this.swiper = new Swiper(this.$refs.kiImageRef, this.SwiperProps)
          }
        ...
        if (this.Value) {
          if (Array.isArray(this.Value)) {
            if (this.Value.length === 0) {
              console.error('value 为空')
            } else if (i < 0 || i > this.Value.length - 1) {
              console.error('preview 参数越界')
            } else {
              this.viewer.view(i)
            }
          } else {
